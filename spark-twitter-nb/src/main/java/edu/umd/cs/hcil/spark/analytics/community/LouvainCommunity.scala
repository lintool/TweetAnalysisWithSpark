...
package edu.umd.cs.hcil.spark.analytics.community
...
/**
  * Implementation of the Louvain Community detection classifier
...
class LouvainCommunity extends Logging with Serializable {
  ...
  /**
    ...
    */
  def louvainFromStandardGraph[VD: ClassTag](sc: SparkContext, graph: Graph[VD, Long], minProgress: Int = 1, progressCounter: Int = 1): (Graph[(VD, Long), Long], Double, Int) = {
    val louvainGraph = createLouvainGraph(graph)
    val (qMeasure, commGraph, count) = louvain(sc, louvainGraph, minProgress, progressCounter)

    val augmentedGraph : Graph[(VD, Long), Long] = graph.outerJoinVertices(commGraph.vertices)((vid, origData, commOpt) => {
      (origData, commOpt.getOrElse(new LouvainData(-1, 0, 0, 0, false)).community)
    })

    (augmentedGraph, qMeasure, count)
  }
...
/**
  ...
  */
class LouvainData(var community: Long, var communitySigmaTot: Long, var internalWeight: Long, var nodeWeight: Long, var changed: Boolean) extends Serializable {

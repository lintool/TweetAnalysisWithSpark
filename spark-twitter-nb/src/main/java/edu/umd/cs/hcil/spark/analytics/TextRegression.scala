...
object TextRegression {
  ...
  /**
    ...
    * @param min Minimum value for regression
    * @param max Maximum value for regression
    * @param sqlContext The SQL context to use for converting to DataFrames
    */
  def convertRdd2Df(samplesRdd : RDD[Sample], min : Double, max : Double, sqlContext : SQLContext) : DataFrame = {
    val meta = NumericAttribute.
      defaultAttr.
      withName("label").
      withMin(min).
      withMax(max).
  ...
  /**
    ...
    */
  def trainModel(samplesRdd : RDD[Sample], min : Double, max : Double, sqlContext : SQLContext, iterations : Int = 10) : Transformer = {
    val samplesWithMeta = convertRdd2Df(samplesRdd, min, max, sqlContext)
  ...
  def trainModel(samplesWithMeta : DataFrame, sqlContext : SQLContext, iterations : Int) : Transformer = {
    ...
    val pipeline = new Pipeline().
      setStages(Array(featureTokenizer, featureCounter, featureIDF, lr))
  ...
  /**
    ...
    */
  def assessClassifier(samplesRdd : RDD[Sample], min: Double, max: Double, sqlContext : SQLContext, runCount : Int, learnerIterations : Int = 10) : (Double, Double) = {

    // Convert samples RDD to DataFrame
    val samples = convertRdd2Df(samplesRdd, min, max, sqlContext)

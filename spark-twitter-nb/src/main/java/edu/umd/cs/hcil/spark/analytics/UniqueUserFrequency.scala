...
object UniqueUserFrequency {
   ...
   * @param timeScale flag for time scale
   */
  def userCounter(tweets : RDD[Status], timeScale: TimeScale.Value) : RDD[(Date,Int)] = {
...
//      (convertTimeToSlice(status.getCreatedAt, timeScale), status.getUser.getId)
//    }).combineByKey(
//        (userId : Long) => {
//          Set[Long](userId)
//        },
//        (l : Set[Long], userId : Long) => {
//          l + userId
//        },
//        (right : Set[Long], left : Set[Long]) => {
//          right ++ left
//        }
//      ).mapValues(s => s.size)

...
object BinaryClassifier {
  ...
  def trainModel(samplesWithMeta : DataFrame, sqlContext : SQLContext, iterations : Int) : Transformer = {
    ...
    val pipeline = new Pipeline().
      setStages(Array(featureTokenizer, featureCounter, featureIDF, gbt))
  ...
  /**
    ...
    */
  def assessClassifier(samplesRdd : RDD[Sample], sqlContext : SQLContext, runCount : Int, learnerIterations : Int = 10) : (Double, Double) = {

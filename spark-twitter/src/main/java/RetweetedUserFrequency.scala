...
object RetweetedUserFrequency {
  ...
  def main(args: Array[String]): Unit = {
    ...
    // Only keep non-null statuses with retweets
    val tweetsFiltered = tweets.filter(status => {
        status != null &&
        status.isRetweet
      })

    val userTuples : RDD[Tuple2[Long, Int]] = tweetsFiltered
      .map(status => (status.getRetweetedStatus.getUser.getId, 1))
      .reduceByKey(_ + _)
      .sortBy(tuple => tuple._2)
    
    // Convert to a CSV string and save
    userTuples.map(tuple => {
        val userId = tuple._1
        val count = tuple._2
        f"$userId%d,\t$count%d"

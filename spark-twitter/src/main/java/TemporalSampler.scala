...
object TemporalSampler {
   ...
   */
  def printHelp() : Unit = {
      println("Usage: spark-submit " + this.getClass.getCanonicalName + "<-m|-h|-d> <input_file> <output_dir> <tweetCount> [numPartitions]")
  ...
  def main(args: Array[String]): Unit = {
    ...
    val tweetsFiltered = tweets.filter(tuple => {
      ...
      line.contains("lang\":\"en\"")
    })

    // For each status, create a tuple with its creation time (flattened to the
    //  correct time scale) and a 1 for counting
    val timedTweets = tweetsFiltered.map(tuple => {
      val status = tuple._2

      (convertTimeToSlice(status.getCreatedAt, timeScale), tuple._1)
    })
    ...
    val numKeys = timeList.length
    val tweetsPerTime = tweetCount.toDouble / numKeys.toDouble
    val keyCounts = timedTweets.countByKey()

    val fractions : Map[Date, Double] = keyCounts.map(tuple => (tuple._1, tweetsPerTime / tuple._2.toDouble)).toMap
    for ( d <- fractions.keys ) {
      println(s"$d -> ${fractions(d)}")
    }

    val downsampled = timedTweets.sampleByKey(false, fractions).map(tuple => tuple._2)

    downsampled.saveAsTextFile(outputPath, classOf[org.apache.hadoop.io.compress.GzipCodec])
  }

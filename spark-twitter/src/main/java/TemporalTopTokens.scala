...
object TemporalTopTokens {
   ...
   */
  def printHelp() : Unit = {
      println("Usage: spark-submit " + this.getClass.getCanonicalName + "<-m|-h|-d> <input_file> <output_dir> <filtered_tokens> <max_tokens> [numPartitions]")
  ...
  def main(args: Array[String]): Unit = {
    ...
    val filterTokenPath = args(3)
    val maxTokens = args(4).toInt

    val filterTokens = scala.io.Source.fromFile(filterTokenPath).getLines.toList
    ...
    //  correct time scale) and the tokens it contains
    val timedTweets = tweetsFiltered.map(status => {
      val tokens = tokenizeAndFilter(filterTokens, status.getText).toSet
      val tokenCounts = tokens.map(token => (token, 1))
      (convertTimeToSlice(status.getCreatedAt, timeScale), tokenCounts)
    })
    ...
    // Implicit ordering for sorting (token, count) tuples
    implicit val tupleValueOrdering = new Ordering[(String, Int)] {
    ...
    var dateTokenMap : Map[Date, Iterable[(String, Int)]] = Map.empty
    for ( time <- timeList ) {

      val thisTimesTweets = timedTweets.filter(tuple => tuple._1.compareTo(time) == 0).flatMap(tuple => tuple._2)
      val tokenCounts = thisTimesTweets.reduceByKey((l, r) => l + r)

      val tokens : Iterable[(String, Int)] = tokenCounts.top(maxTokens)

      dateTokenMap = dateTokenMap + (time -> tokens)
    }

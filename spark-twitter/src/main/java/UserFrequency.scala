...
object UserFrequency {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val conf = new SparkConf().setAppName("User Frequency")
    ...
    val timedTweets = tweetsFiltered.map(status =>
      (convertTimeToSlice(status.getCreatedAt, timeScale), List(status.getUser.getId)))

    val groupedTweets = timedTweets.reduceByKey((l, r) => l ++ r)
    ...
    val fullTimesRdd : RDD[(Date, List[Long])] =
      sc.parallelize(timeList).map(key => (key, List.empty[Long]))

    // Merge the full date list and regular data
    val withFullDates = groupedTweets.union(fullTimesRdd)
    val mergedDates = withFullDates.reduceByKey((l, r) => l ++ r)

    val dateCounts = mergedDates.mapValues(listOfUsers => listOfUsers.distinct.size)

    // Sort for printing
    val sliceCounts = dateCounts.sortByKey()
...
//    timedTweets.cache()
//
//    var dateUserMap : Map[Date, Long] = Map.empty
//    for ( time <- timeList ) {
//
//      val thisTimesTweets : RDD[Long] = timedTweets
//        .filter(tuple => tuple._1.compareTo(time) == 0)
//        .map(tuple => tuple._2)
//
//      val userCounts = thisTimesTweets.distinct.count
//
//      dateUserMap = dateUserMap + (time -> userCounts)
//    }
//
...
//      val elements : List[String] = List(dateString, dateUserMap(time).toString)
//
